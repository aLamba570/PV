...
def train_models(df, features, target='ppc_p_tot'):
    ...
    # Impute missing values for LinearRegression
    ...
    # HistGradientBoostingRegressor (can handle NaNs)
...
if st.sidebar.button("Load and Analyze Data"):
    ...
    **Deliverables:**
    - CSV of per-15-min loss flags and values
    - Charts: loss breakdown over time
    - Dashboard: Streamlit-based interface for exploring insights interactively

    **Suggestions:**
    - Include string/inverter IDs for asset-level diagnostics
    - Add module-level soiling sensors for real-time soiling detection
    - Improve temperature sensor calibration and coverage
